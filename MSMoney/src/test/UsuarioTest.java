...
public class UsuarioTest {
	public static void main(String[] args) {
		...
		Financa financa3 = new Financa("Combustivel","Automovel", true, 400.0,FinancaEnum.DESPESAS);
		usuario.setDespesas(usuario.usuarioUtils.inserirFinanca(usuario.getDespesas(), financa3));
	
		Financa financa4 = new Financa("Poupança", true, 500.0,FinancaEnum.LONGO_PRAZO);
		usuario.setLongoPrazo(usuario.usuarioUtils.inserirFinanca(usuario.getLongoPrazo(), financa4));
		
		Financa financa5 = new Financa("IPVA", true, 300.0,FinancaEnum.DESPESAS_OCASIONAIS,1);
		usuario.setDespesasOcasionais(usuario.usuarioUtils.inserirFinanca(usuario.getDespesasOcasionais(), financa5));
		...
		usuario.setResumo(usuario.usuarioUtils.atualizarResumo(usuario.getResumo(), usuario.getRendimentos(), usuario.getDespesas(), usuario.getLongoPrazo(),usuario.getDespesasOcasionais()));
		for(int i=0;i<12;i++) {
